package tfr.game.jam.kittyescape.model.network
...
class NetworkBridge(
    val bus: EventBus, val scope: CoroutineScope, val world: World, var userName: String = "unknown"
) : AsyncDependency {
    ...
    val allowedEvents = setOf(
        Action.HeroLeft, Action.HeroRight, Action.HeroUp, Action.HeroDown
    )
    ...
    fun executePlayerUpdate(updateCommand: String) {
        ...
        update.players.forEach { player ->/*world.getPlayer(player.number)?.let { playerPos ->
